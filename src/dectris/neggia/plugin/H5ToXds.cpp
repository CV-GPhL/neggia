...
// CV-20210330: this is incorrect for processing. Overflows need to be
// marked as such and not as if they were invalid pixels!

template <class T>
int32_t applyOverflowForXds(T value, const size_t countCutoff);

template <>
int32_t applyOverflowForXds<uint32_t>(uint32_t value, const size_t countCutoff) {
    // XDS uses int32_t pixel values for processing therefore
    // cannot use any pixels from uint32_t >= 2**31
    // these values must be set to count_cutoff.
    return (value > INT32_MAX) ? countCutoff : value;
}

template <>
int32_t applyOverflowForXds<uint16_t>(uint16_t value, const size_t countCutoff) {
    // For conversion from uint16_t we only need to set the
    // 'overflow' value of 0xFFFF to count_cutoff. All other values
    // from uint16_t are allowed.
    return (value == 0xFFFF) ? countCutoff : value;
}

template <>
int32_t applyOverflowForXds<uint8_t>(uint8_t value, const size_t countCutoff) {
    // For conversion from uint8_t we only need to set the
    // 'overflow' value of 0xFF to count_cutoff. All other values
    // from uint8_t are allowed.
    return (value == 0xFF) ? countCutoff : value;
}
...
template <class T>
void applyMaskAndTransformToInt32ForXds(const T* indata,
                                  int outdata[],
                                  const int32_t* mask,
                                  size_t size, size_t countCutoff) {
    for (size_t j = 0; j < size; ++j) {
        outdata[j] = mask[j] ? mask[j] : applyOverflowForXds(indata[j], countCutoff);
    }
}
...
void setCountCutoff(H5DataCache* dataCache) {
    try {
        Dataset d(dataCache->h5File, "/entry/instrument/detector/detectorSpecific/countrate_correction_count_cutoff");
        dataCache->CountCutoff = readNonZeroUint(d);
    } catch (const std::out_of_range&) {
        dataCache->CountCutoff = 0;
    }
    std::cout << std::endl << " [dectris-neggia-plugin] countrate_correction_count_cutoff = " << dataCache->CountCutoff << std::endl << std::endl;
}
...
void applyMaskAndTransformToInt32ForXds(const H5DataCache* dataCache,
                                  const void* indata,
                                  int outdata[]) {
    switch (dataCache->datasize) {
        case 1:
            applyMaskAndTransformToInt32ForXds((const uint8_t*)indata, outdata,
                                         dataCache->mask.get(),
                                         dataCache->dimx * dataCache->dimy,
                                         dataCache->CountCutoff);
            break;
        case 2:
            applyMaskAndTransformToInt32ForXds((const uint16_t*)indata, outdata,
                                         dataCache->mask.get(),
                                         dataCache->dimx * dataCache->dimy,
                                         dataCache->CountCutoff);
            break;
        case 4:
            applyMaskAndTransformToInt32ForXds((const uint32_t*)indata, outdata,
                                         dataCache->mask.get(),
                                         dataCache->dimx * dataCache->dimy,
                                         dataCache->CountCutoff);
            break;
...
void readDataset(int* frame_number,
    ...
    try {
        ...
        applyMaskAndTransformToInt32ForXds(dataCache, buffer.get(), data_array);
    } catch (const std::out_of_range&) {
